...
(function($) {
  $.fn.hoverAccordion = function(options) {
    ...
    function doHover(itemList, itemHeader, listHeight) {
      ...
      var oldList = $(container).find('.' + options.classOpen).closest('li').find('ul:first');
      ...
      if (false === oldList.is(':animated')) {
        ...
        if (itemHeader.hasClass(options.classOpen) == false) {
          ...
          oldList.closest('li').removeClass(options.classActive).find('a:first').addClass(options.classClosed).removeClass(
          options.classOpen);
        }
      }
    }
    ...
    function() {
      ...
      if (itemHeader.length > 0) {
        ...
        } else if (options.activateItem == true && pageHref.indexOf(itemHref) > 0 && pageHref.length - pageHref.lastIndexOf(itemHref) == itemHref.length) {
          isActive = true;
      ...
      if (itemList.length > 0) {
        ...
        if (options.activateItem === true) {
          itemList.children('li').each(

          function() {
            var m = $(this).find('a').attr('href');
            if (m) {
              if (pageHref.indexOf(m) > 0 && pageHref.length - pageHref.lastIndexOf(m) == m.length) {
                isActive = true;
                item.addClass(options.classActive);
